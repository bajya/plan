...
(function($) {
  ...
  $.fn.maskAsNumber = function(options){
    ...
    this.each(function(idx, element){
      ...
      if(!(/\d\./.test(value))){ $elem.val(''); return; }
      ...
      if(data.receivedMinus && char === '-') {
        ...
        data.receivedMinus = false;
      } else { data.receivedMinus = false; char === '-'; }
      if(data.decimals){
      ...
      if (data.min !== '' && data.min[0] !== '-' && data.receivedMinus){ event.preventDefault(); }
        ...
        event.preventDefault();
    }).on('keydown.'+dataKey, function(event){
      let $this = $(this), data = $this.data(dataKey);
      data.receivedMinus = false;
      // alert(data.receivedMinus);
      //if(data.decimals > 0)
      //{
      ...
      {
        if(!data.receivedMinus){ return; }
      }*/
      // console.log('receivedMinus',data.receivedMinus);
