...
@section('title', 'Allow States')

@section('content')
	<div class="content-wrapper">
        <div class="row page-titles">
            <div class="col-md-5 align-self-center">
                <h3 class="text-primary">Allow States</h3> </div>
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="{{route('dashboard')}}">Home</a></li>
                    <li class="breadcrumb-item active">Allow States</li>
                </ol>
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <h4 class="card-title">Allow States</h4>

                            <div class="dt-buttons float-right">
                                <a href="{{route('createAllowState')}}" class="btn dt-button py-2">Add Allow State</a>
                            </div>
                        
                        <div class="table-responsive m-t-40">
                            <div class="dt-buttons">
                                <a href="javascript:void(0)" data-href="{{route('deleteAllowStates')}}" class="btn btn-secondary disabled bulkAction deleteAllowStates">Delete</a>
                            </div>
                            <div class="dt-buttons">
                                <a href="javascript:void(0)" data-href="{{route('changeStatusAllowStates')}}" class="btn btn-secondary disabled bulkAction changeStatusAllowStates">Activate/Deactivate</a>
                            </div>
                            <table id="allowstatesTable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form-valide" method="post" id="blockForm" action="{{route('changeStatusAllowState')}}">
                        {{csrf_field()}}
                        <input type="hidden" name="statusid" id="statusid">
                        <input type="hidden" name="status" id="status">
                        <h5 class="m-t-10 text-danger">Are you sure you want to <span class="allowstatestatus"></span> Allow State : <span id="statuscode"></span></h5>
    ...
    <div class="modal fade" id="confirmDeleteModal" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{-- <form class="form-valide" method="post" id="deleteForm" action="{{route('deleteAllowState')}}"> --}}
                        {{csrf_field()}}
                        <input type="hidden" name="deleteid" id="deleteid">
                        <input type="hidden" name="deleteurl" id="deleteurl">
                        <h5 class="m-t-10 text-danger">Are you sure you want to delete selected Allow State?</h5>
                        <button type="button" class="btn btn-secondary btn-flat cancelBtn m-b-30 m-t-30" data-dismiss="modal">Cancel</button>
    ...
    <script type="text/javascript">
    	$(function(){
            ...
            function myDataTableFunction(){
                table = $('#allowstatesTable').DataTable({
                    "ajax": {
                        url: "{{route('allowstateAjax')}}",
                        dataSrc:"data",
                        type:"POST",
                        data:{
...
lengthChange: true,
            ...
            if ($.fn.DataTable.isDataTable("#allowstatesTable")) {
                $('#allowstatesTable').DataTable().clear().destroy();
            }
            myDataTableFunction();
            ...
            $(document).on('click','input[name="state_ids[]"]',function(){
                ...
                if(length > 0)
                {
                    $('.deleteAllowStates').removeClass('disabled');
                    $('.changeStatusAllowStates').removeClass('disabled');
                    table.rows().select();
                }else{
                    $('.deleteAllowStates').addClass('disabled');
                    $('.changeStatusAllowStates').addClass('disabled');
                    table.rows().deselect();
                }
            });
            ...
            $(document).on('switchChange.bootstrapSwitch', '.statusAllowState', function (event, state) {
                var x;
                ...
                if(x == 'inactive')
                {
                    $('#statusid').val(id);
                    $('#status').val('inactive');
                    $('.allowstatestatus').text('deactivate');
                }
                else
                {
                    $('#statusid').val(id);
                    $('#status').val('active');
                    $('.allowstatestatus').text('activate');
                }

                $.ajax({
                    type: "post",
                    url: "{{route('changeStatusAjaxAllowState')}}",
                    ...
                    error: function(data)
                    {
                        ...
                        toastr.error("Unable to update Allow State.","Status",{
                            timeOut: 5000,
                            "closeButton": true,
                            "debug": false,
                            "newestOnTop": true,
            ...
            $(document).on('click','.deleteAllowState',function(){
                var id = $(this).data('id');
                $('#deleteid').val(id);
                $('#deleteurl').val("{{route('deleteAllowState')}}");
            ...
            $(document).on('click', '.confirmDeleteBtn', function (event, state) {
                ...
                $.ajax({
                    ...
                    error: function(data)
                    {
                        ...
                        toastr.error("Unable to delete Allow State.","Status",{
                            timeOut: 5000,
                            "closeButton": true,
                            "debug": false,
                            "newestOnTop": true,
                ...
                if(url.includes('delete')){
                    ...
                    $.ajax({
                        ...
                        error: function(data)
                        {
                            ...
                            toastr.error("Unable to update Allow States.","Status",{
                                timeOut: 5000,
                                "closeButton": true,
                                "debug": false,
                                "newestOnTop": true,
