...
@section('title', ucfirst($type).' State')
        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{ucfirst($type)}} State</li>
                </ol>
            </div>
        </div> 
        <!-- Start Page Content -->
            ...
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        @if($type == 'add')
                            <h4>Fill In State Details</h4>
                        @elseif($type == 'edit')
                            <h4>Edit State Details</h4>
                        @endif
                        <hr>
                        @if ($errors->any())
                        ...
                        <form class="form-material m-t-50 row form-valide" method="post" action="{{$url}}" enctype="multipart/form-data">
                            ...
                            <div class="col-md-12 p-0">
                                ...
                                <div class="form-group col-md-6 m-t-20 float-left">
                                    ...
                                    <input type="text" class="form-control form-control-line" name="state_name" value="{{old('state_name', $state->name)}}" maxlength="100">
                                </div>
                                <input type="hidden" name="status" value="@if(isset($state) && $state->status != null) {{$state->status}} @else active @endif">
                                <div class="form-group bt-switch col-md-6 m-t-20">
                                    <label class="col-md-4">Status</label>
                                    <div class="col-md-3" style="float: right;">
                                        <input type="checkbox" @if($type == 'edit') @if(isset($state) && $state->status == 'active') checked @endif @else checked @endif data-on-color="success" data-off-color="info" data-on-text="Active" data-off-text="Inactive" data-size="mini" name="val-status" id="statusCat">
                                    </div>
                                </div>
                            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h5 class="m-t-10 text-danger changeOffer">Are you sure you want to removed State?.</h5>
        ...
        $(function(){
            ...
            @if($type == 'edit')
               /* $('input[name=state_name]').rules('add', {remote: {
                    url: APP_NAME + "/admin/states/checkState/{{$state->id}}",
                    type: "post",
                    data: {
                      parent: function() {
            ...
            @else
                /*$('input[name=state_name]').rules('add', {remote: {
                    url: APP_NAME + "/admin/states/checkState",
                    type: "post",
                    data: {
                      parent: function() {
                        return $( "#parent_id" ).val();
