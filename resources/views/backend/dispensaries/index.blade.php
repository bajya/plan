                    ...
                    <div class="card-body">
                        <h4 class="card-title">Locations</h4>
                            <div class="dt-buttons float-right">
                                <a href="{{route('importDispensary')}}" class="btn dt-button py-2">Bulk Import</a>
                            </div>
                            <div class="dt-buttons float-right">
                                <a href="{{route('createDispensary')}}" class="btn dt-button py-2">Add Location</a>
                            </div>
                        
                        <div class="table-responsive m-t-40">
                            <div class="dt-buttons">
                                <a href="javascript:void(0)" data-href="{{route('deleteDispensaries')}}" class="btn btn-secondary disabled bulkAction deleteDispensaries">Delete</a>
                            </div>
                            <div class="dt-buttons">
                                <a href="javascript:void(0)" data-href="{{route('changeStatusDispensaries')}}" class="btn btn-secondary disabled bulkAction changeStatusDispensaries">Activate/Deactivate</a>
                            </div>
                            <table id="dispensariesTable" class="display nowrap table table-hover table-striped table-bordered" cellspacing="0" width="100%">
                                <thead>
                                    <tr>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form class="form-valide" method="post" id="blockForm" action="{{route('changeStatusDispensary')}}">
                        {{csrf_field()}}
                        <input type="hidden" name="statusid" id="statusid">
                        <input type="hidden" name="status" id="status">
                        <h5 class="m-t-10 text-danger">Are you sure you want to <span class="dispensarystatus"></span> Location : <span id="statuscode"></span></h5>
    ...
    <div class="modal fade" id="confirmDeleteModal" tabindex="-1" role="dialog" aria-labelledby="confirmDeleteModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    {{-- <form class="form-valide" method="post" id="deleteForm" action="{{route('deleteDispensary')}}"> --}}
                        {{csrf_field()}}
                        <input type="hidden" name="deleteid" id="deleteid">
                        <input type="hidden" name="deleteurl" id="deleteurl">
                        <h5 class="m-t-10 text-danger">Are you sure you want to delete selected Location?</h5>
                        <button type="button" class="btn btn-secondary btn-flat cancelBtn m-b-30 m-t-30" data-dismiss="modal">Cancel</button>
    ...
    <script type="text/javascript">
    	$(function(){
            ...
            function myDataTableFunction(){
                table = $('#dispensariesTable').DataTable({
                    "ajax": {
                        url: "{{route('dispensaryAjax')}}",
                        dataSrc:"data",
                        type:"POST",
                        data:{
            ...
            if ($.fn.DataTable.isDataTable("#dispensariesTable")) {
                $('#dispensariesTable').DataTable().clear().destroy();
            }
            myDataTableFunction();
            ...
            $(document).on('click','input[name="user_ids[]"]',function(){
                ...
                if(length > 0)
                {
                    $('.deleteDispensaries').removeClass('disabled');
                    $('.changeStatusDispensaries').removeClass('disabled');
                    table.rows().select();
                }else{
                    $('.deleteDispensaries').addClass('disabled');
                    $('.changeStatusDispensaries').addClass('disabled');
                    table.rows().deselect();
                }
            });
            ...
            $(document).on('switchChange.bootstrapSwitch', '.statusDispensary', function (event, state) {
                var x;
                ...
                if(x == 'inactive')
                {
                    $('#statusid').val(id);
                    $('#status').val('inactive');
                    $('.dispensarystatus').text('deactivate');
                }
                else
                {
                    $('#statusid').val(id);
                    $('#status').val('active');
                    $('.dispensarystatus').text('activate');
                }

                $.ajax({
                    type: "post",
                    url: "{{route('changeStatusAjaxDispensary')}}",
                    ...
                    error: function(data)
                    {
                        ...
                        toastr.error("Unable to update Location.","Status",{
                            timeOut: 5000,
                            "closeButton": true,
                            "debug": false,
                            "newestOnTop": true,
            ...
            $(document).on('click','.deleteDispensary',function(){
                var id = $(this).data('id');
                $('#deleteid').val(id);
                $('#deleteurl').val("{{route('deleteDispensary')}}");
            ...
            $(document).on('click', '.confirmDeleteBtn', function (event, state) {
                ...
                $.ajax({
                    ...
                    error: function(data)
                    {
                        ...
                        toastr.error("Unable to delete Location.","Status",{
                            timeOut: 5000,
                            "closeButton": true,
                            "debug": false,
                            "newestOnTop": true,
                ...
                if(url.includes('delete')){
                    ...
                    $.ajax({
                        ...
                        error: function(data)
                        {
                            ...
                            toastr.error("Unable to update dispensaries.","Status",{
                                timeOut: 5000,
                                "closeButton": true,
                                "debug": false,
                                "newestOnTop": true,
