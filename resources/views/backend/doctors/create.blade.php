...
@section('title', ucfirst($type).' Doctor')
        ...
        <div class="row page-titles">
            ...
            <div class="col-md-7 align-self-center">
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active">{{ucfirst($type)}} Doctor</li>
                </ol>
            </div>
        </div>
        <!-- Start Page Content -->
                        ...
                        <form class="form-material m-t-50 row form-valide" method="post" action="{{$url}}" enctype="multipart/form-data">
                            ...
                            <div class="col-md-12 p-0">
                                <div class="form-group col-md-6 m-t-20 float-left">
                                    ...
                                    @if($type == 'add' || ($type == 'edit' && $doctor->image == null))
                                        <div class="fileinput fileinput-new input-group" data-provides="fileinput">
                                            ...
                                            <input type="file" required name="doc_image" accept=".jpg, .jpeg, .png"> </span> <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">Remove</a>
                                        </div>
                                    @elseif($type == 'edit')
                                        <br>
                                        <div id="catImage">

                                            <img src="@if($doctor->image != null && file_exists(public_path('/uploads/doctors/'.$doctor->image))){{URL::asset('/uploads/doctors/'.$doctor->image)}}@endif" width="70"" />
                                ...
                                <div class="form-group col-md-6 m-t-20">
                                    ...
                                    <input type="text" class="form-control form-control-line" name="doc_name" value="{{old('doc_name', $doctor->name)}}" maxlength="100">
                                </div> 
                            ...
                            <div class="form-group col-md-6 m-t-20" id="brandcol" style="display: none;">
                                    ...
                                    @if(count($brands) > 0)
                                        ...
                                        @if($brands)
                                            @foreach($brands as $id=> $des)
                                                <option value="{{$des->id}}" @if($doctor->brand_id==$des->id) selected @endif>{{ ucfirst($des->name) }}</option>
                                            @endforeach
                                        @endif
                                    @else
                             ...
                             <div class="form-group col-md-6 m-t-20">
                                ...
                                <input type="text" class="form-control form-control-line" name="phone_number" value="{{old('phone_number', $doctor->phone_number)}}" placeholder="Please enter phone number">
                            </div>
                            <div class="form-group col-md-6 m-t-20">
                                <label>Email</label><sup class="text-reddit"> *</sup>
                                <input type="text" class="form-control form-control-line" name="doc_email" value="{{old('email', $doctor->email)}}" placeholder="Please enter email">
                            </div>
                            <div class="form-group col-md-6 m-t-20">
                                <label>Zip Code</label><sup class="text-reddit"> *</sup>
                                <input type="text" class="form-control form-control-line" name="zipcode" value="{{old('zipcode', $doctor->zipcode)}}" placeholder="Please enter zipcode">
                            </div>
                            <div class="form-group col-md-12 m-t-20">
                                <label>Location/City/Address</label><sup class="text-reddit"> *</sup>
                                <input type="text" name="address" id="address" value="{{old('address', $doctor->address)}}" class="form-control form-control-line" placeholder="">
                                <input type="hidden" id="country" name="country" class="form-control form-control-line" value="{{ isset($doctor->country) && !empty($doctor->country) ? $doctor->country : ''}}">
                                <input type="hidden" id="state" name="state" class="form-control form-control-line" value="{{ isset($doctor->state) && !empty($doctor->state) ? $doctor->state : ''}}">
                                <input type="hidden" id="city" name="city" class="form-control form-control-line" value="{{ isset($doctor->city) && !empty($doctor->city) ? $doctor->city : ''}}">
                            </div>
                            ...
                            <div class="col-md-12 p-0" style="display: none;">
                                <div class="form-group col-md-6 m-t-20 float-left" id="latitudeArea">
                                    ...
                                    <input type="text" id="latitude" name="lat" class="form-control form-control-line" value="{{ isset($doctor->lat) && !empty($doctor->lat) ? $doctor->lat : '-7.0157404'}}" readonly>
                                </div> 
                                <div class="form-group col-md-6 m-t-20" id="longtitudeArea">
                                    <label>Longitude</label><sup class="text-reddit"> *</sup>
                                    <input type="text" name="lng" id="longitude" class="form-control form-control-line" value="{{ isset($doctor->lng) && !empty($doctor->lng) ? $doctor->lng : '110.4171283'}}" readonly>
                                </div>
                            </div>

                            <input type="hidden" name="status" value="@if(isset($doctor) && $doctor->status != null) {{$doctor->status}} @else active @endif">
                            <div class="form-group bt-switch col-md-6 m-t-20">
                                <label class="col-md-4">Status</label>
                                <div class="col-md-3" style="float: right;">
                                    <input type="checkbox" @if($type == 'edit') @if(isset($doctor) && $doctor->status == 'active') checked @endif @else checked @endif data-on-color="success" data-off-color="info" data-on-text="Active" data-off-text="Inactive" data-size="mini" name="val-status" id="statusCat">
                                </div>
                            </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <h5 class="m-t-10 text-danger changeOffer">Are you sure you want to removed Doctor?.</h5>
    ...
    <script type="text/javascript">
        $(function(){
            ...
            $('#changeImage').click(function(){
                $('#catImage').parent().append('<div class="fileinput fileinput-new input-group" data-provides="fileinput"><div class="form-control" data-trigger="fileinput"> <i class="glyphbanner glyphbanner-file fileinput-exists"></i> <span class="fileinput-filename"></span></div> <span class="input-group-addon btn btn-default btn-file"> <span class="fileinput-new">Select file(Allowed Extensions -  .jpg, .jpeg, .png, .gif, .svg)</span> <span class="fileinput-exists">Change</span><input type="file" required name="doc_image" accept=".jpg, .jpeg, .png"> </span> <a href="#" class="input-group-addon btn btn-default fileinput-exists" data-dismiss="fileinput">Remove</a></div>');
            ...
            @if($type == 'edit')
                $('input[name=doc_name]').rules('add', {remote: {
                    url: APP_NAME + "/admin/doctors/checkDoctor/{{$doctor->id}}",
                    type: "post",
                    data: {
                      parent: function() {
            ...
            @else
                $('input[name=doc_name]').rules('add', {remote: {
                    url: APP_NAME + "/admin/doctors/checkDoctor",
                    type: "post",
                    data: {
                      parent: function() {
                        return $( "#parent_id" ).val();
